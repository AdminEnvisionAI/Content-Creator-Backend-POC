...
# ... analyze_thumbnail_colors, analyze_visual_keywords_instagram, calculate_visual_aesthetics_score_instagram functions yahan aayenge (koi badlaav nahi) ...
def analyze_thumbnail_colors(image_url: str) -> List[str]:
    print("image_url----------------->", image_url)
...
def calculate_visual_aesthetics_score_instagram(post_data: dict) -> int:
    ...
    total_score += min(keyword_score, 50)
    colors = post_data.get("dominant_colors", [])
    if len(colors) >= 5: total_score += 50
    elif len(colors) >= 3: total_score += 25
    return min(total_score, 100)
    
### NAYA HELPER FUNCTION: User ke Access Token se IG Business ID nikalna ###
def get_instagram_business_id(user_access_token: str) -> Optional[str]:
    """
    User ke Access Token ka istemaal karke uske Facebook Pages se jude
    Instagram Business Account ki ID nikalta hai.
    Zaroori Permissions: pages_show_list, instagram_basic
    """
    base_url = os.getenv("IG_BASE_URL", "https://graph.facebook.com/v19.0")
    url = f"{base_url}/me/accounts"
    
    # Hum `instagram_business_account` field maang rahe hain
    params = {
        'fields': 'name,instagram_business_account',
        'access_token': user_access_token
    }
    
    print("User ke pages se Instagram ID dhoond rahe hain...")
    try:
        response = requests.get(url, params=params)
        response.raise_for_status()
        data = response.json()
        
        # Response mein aaye saare pages ko check karein
        for page in data.get('data', []):
            if 'instagram_business_account' in page:
                ig_id = page['instagram_business_account']['id']
                page_name = page['name']
                print(f"✅ Instagram Business ID mila: {ig_id} (Facebook Page: '{page_name}')")
                # Jaise hi pehla ID milta hai, use return kar dein
                return ig_id
        
        print("❌ Is user ke kisi bhi Facebook Page se koi Instagram Business Account nahi juda hai.")
        return None
        
    except requests.exceptions.RequestException as e:
        print(f"API call karte samay error aaya: {e}")
        # Agar error response mein details hain, to woh bhi print karein
...
def fetch_instagram_data(target_username: str, posts_limit: int = 10):
    ...
    access_token = os.getenv("IG_ACCESS_TOKEN")
    base_url = os.getenv("IG_BASE_URL")
    if not business_id or not access_token: raise ValueError("IG_BUSINESS_ID ya IG_ACCESS_TOKEN env vars mein nahi mile.")
    url = f'{base_url}/{business_id}'
    fields = (f'business_discovery.username({target_username}){{id,username,name,followers_count,media_count,biography,profile_picture_url,'
              f'media.limit({posts_limit}){{id,caption,like_count,comments_count,timestamp,media_type,media_url,thumbnail_url,duration}}}}')
    params = {'fields': fields, 'access_token': access_token}
    try:
        response = requests.get(url, params=params, timeout=30)
        response.raise_for_status()
        data = response.json()
        print("data (via business_discovery)-------------->", data)
        if 'business_discovery' in data: return data['business_discovery']
        elif 'error' in data: raise Exception(f"Instagram API Error: {data['error'].get('message', 'Unknown error')}")
        else: raise Exception("API se unexpected response mila.")
    except requests.exceptions.RequestException as e:
        raise Exception(f"Network error: {str(e)}")


def fetch_instagram_data_with_insights(ig_user_id: str, user_access_token: str, posts_limit: int = 10):
    # ... Is function mein koi badlaav nahi ...
    base_url = os.getenv("IG_BASE_URL")
    if not ig_user_id or not user_access_token: raise ValueError("Instagram User ID ya User Access Token nahi mila.")
    url = f'{base_url}/{ig_user_id}'
    fields = ('id,username,name,followers_count,media_count,biography,profile_picture_url,'
              f'media.limit({posts_limit}){{id,caption,like_count,comments_count,timestamp,media_type,media_url,thumbnail_url,views_count,play_count,duration}}')
    params = {'fields': fields, 'access_token': user_access_token}
    try:
        response = requests.get(url, params=params, timeout=30)
        response.raise_for_status()
        data = response.json()
        print("data (with insights)-------------->", data)
...
def calculate_instagram_metrics(profile_data: dict) -> Metrics:
    ...
    if not posts or followers == 0:
        visual_scores = [p.get("visual_aesthetics_score", 0) for p in posts if "visual_aesthetics_score" in p]
        avg_visual_score = sum(visual_scores) / len(visual_scores) if visual_scores else 0.0
        return Metrics(engagement_rate_per_post=0.0, like_comment_ratio=0.0, post_frequency_per_week=0.0,
                       sentiment_score=0.0, overall_score=0.0, avg_visual_score=round(avg_visual_score, 2))
    total_likes = sum(p.get('likes', 0) for p in posts)
    total_comments = sum(p.get('comments_total', 0) for p in posts)
    avg_engagement_per_post = (total_likes + total_comments) / len(posts)
    engagement_rate = (avg_engagement_per_post / followers * 100)
    ...
    visual_scores = [p.get("visual_aesthetics_score", 0) for p in posts if "visual_aesthetics_score" in p]
    avg_visual_score = sum(visual_scores) / len(visual_scores) if visual_scores else 0.0
    return Metrics(engagement_rate_per_post=round(engagement_rate, 4), like_comment_ratio=round(like_comment_ratio, 2),
                   post_frequency_per_week=round(post_frequency, 2), sentiment_score=round(sentiment_score, 2),
                   overall_score=round(overall_score, 2), avg_visual_score=round(avg_visual_score, 2))

# ==============================================================================
# 3. MAIN PROCESSING FUNCTIONS (Sabko Jodkar Kaam Karna)
# ==============================================================================

async def _process_and_save_profile(data: dict, username_or_id: str):
    # ... Is helper function mein koi badlaav nahi ...
    loop = asyncio.get_running_loop()
    media_items = data.get('media', {}).get('data', [])
    if not media_items:
        print(f"'{username_or_id}' ke liye koi media nahi mila."); return
    processed_posts = []
    for post in media_items:
        media_url = post.get('thumbnail_url') or post.get('media_url')
        caption = post.get('caption', '')
        ...
        visual_tags = analyze_visual_keywords_instagram(caption)
        temp_post_data = {
            "post_id": post.get('id'), "title": caption,
            "published_at": datetime.strptime(post['timestamp'], "%Y-%m-%dT%H:%M:%S%z"),
            "views": post.get('views_count'), "likes": post.get('like_count'), "comments_total": post.get('comments_count'),
            "media_url": media_url, "dominant_colors": dominant_colors, "visual_style_tags": visual_tags,
            "thumbnail_url": media_url, "media_type": post.get('media_type')
        }
        temp_post_data["visual_aesthetics_score"] = calculate_visual_aesthetics_score_instagram(temp_post_data)
        processed_posts.append(temp_post_data)
    profile_data_for_metrics = {"followers_count": data.get('followers_count'), "posts": processed_posts}
    metrics = calculate_instagram_metrics(profile_data_for_metrics)
    profile = InfluencerProfile(platform_id=data.get('id'), platform="instagram", name=data.get('name', username_or_id),
                              username=data.get('username'), profile_pic_url=data.get('profile_picture_url'),
                              bio=data.get('biography'), followers=data.get('followers_count'),
                              posts=[PostStats(**p) for p in processed_posts], metrics=metrics)
    await profile.save()
    print(f"✅ Saved Instagram profile for: {profile.name} with score {profile.metrics.overall_score} and visual score {profile.metrics.avg_visual_score}")
    return profile
...
async def process_instagram_profile(username: str, posts_limit: int = 10):
    ...
    try:
        data = await loop.run_in_executor(None, fetch_instagram_data, username, posts_limit)
        return await _process_and_save_profile(data, username)
    except Exception as e:
        import traceback; traceback.print_exc()
        print(f"❌ Public Instagram profile process karte samay error ({username}): {e}")
        return {"error": str(e)}

async def process_authenticated_instagram_profile(ig_user_id: str, user_access_token: str, posts_limit: int = 10):
    # ... Is function mein koi badlaav nahi ...
    loop = asyncio.get_running_loop()
    try:
        data = await loop.run_in_executor(None, fetch_instagram_data_with_insights, ig_user_id, user_access_token, posts_limit)
        return await _process_and_save_profile(data, ig_user_id)
    except Exception as e:
        import traceback; traceback.print_exc()
        print(f"❌ Authenticated Instagram profile process karte samay error ({ig_user_id}): {e}")
        return {"error": str(e)}

### NAYA MAIN FLOW FUNCTION: Poore authenticated flow ko chalane ke liye ###
async def run_full_authenticated_flow(user_access_token: str, posts_limit: int = 2):
    """
    Yeh function poore authenticated flow ko chalata hai:
    1. User ke token se IG Business ID nikalta hai.
    2. Agar ID mil jaata hai, to uss ID se profile data fetch aur process karta hai.
    """
    user_access_token="EAAcFTID6DLsBP2uNWnUURV3uNVHZCSQFnJp3V5BJ2kftUmbGr8KV7BAmRjSFmXjsMUB4ZBtGDBS2gCZCEwAUOSrgFHeKNdWmEyQdWs9yt8CZCdTWJa2luEBl2ZA6Hqw2L1tsLHgPcQdQXZBjn51tQVtWvrnfQPtHtwAaC6yLN3kncBCcIyvU3FHU8x9D2ZA"
    loop = asyncio.get_running_loop()
    
    # Step 1: Sync function `get_instagram_business_id` ko async context mein call karein
    ig_user_id = await loop.run_in_executor(None, get_instagram_business_id, user_access_token)
    
    # Step 2: Check karein ki ID mila ya nahi
    if ig_user_id:
        # Step 3: Agar ID mil gaya, to profile ko process karein
        print(f"Processing profile for IG User ID: {ig_user_id}")
        profile_data = await process_authenticated_instagram_profile(
        ...
        # Agar ID nahi mila, to error message return karein
        error_message = "Could not find an Instagram Business Account linked to the user's Facebook pages."
        print(error_message)
